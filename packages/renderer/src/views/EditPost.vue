    ...
    <div class="noselect flex flex-col mt-8">
        <div class="py-2 -my-2 overflow-x-auto sm:-mx-6 sm:px-6 lg:-mx-8 lg:px-8">
            <div class="grid grid-cols-1 gap-6">
                ...
                <label class="block">
                    <span class="text-gray-700">Ссылка</span>
                    <input
                        v-model="cat.link"
                        type="text"
                        class="mt-1 block w-full rounded-md border-gray-300 shadow-sm focus:border-indigo-300 focus:ring focus:ring-indigo-200 focus:ring-opacity-50"
                    />
                    <span
                        v-if="!cat.link && checked"
                        class="flex items-center font-medium tracking-wide text-red-500 mt-1 ml-1"
                    >Поле не может быть пустым!</span>
                </label>
                <label class="block">
                    <span class="text-gray-700">Интерес</span>
                    <select
                        v-model="cat.interest_id"
                        class="mt-1 block w-full rounded-md border-gray-300 shadow-sm focus:border-indigo-300 focus:ring focus:ring-indigo-200 focus:ring-opacity-50"
                    >
                        <option
                            v-for="categ in categs"
                            :value="categ.id"
                            :selected="categ.title == cat.interest"
                        >{{ categ.title }}</option>
                    </select>
                </label>
                <label class="block">
...
export default {
    ...
    methods: {
        ...
        createCat() {
            ...
            if (!this.cat.link) return
            delete this.cat.interest
            axios.put('http://109.254.85.64/newsletter/api/db/posts/' + this.id, this.cat).then((resp) => {
                console.log(resp)
            }).catch((e) => {
    ...
    async mounted() {
        this.cat = (await axios.get('http://109.254.85.64/newsletter/api/db/posts/' + this.id)).data
