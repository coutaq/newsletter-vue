...
export default {
    ...
    methods: {
        ...
        createUser() {
            ...
            axios.put('http://109.254.85.64/newsletter/api/db/dbusers/' + this.id, this.cat).then((resp) => {
                console.log(resp)
            }).catch((e) => {
    ...
    created() {
        var self = this
        axios.get('http://109.254.85.64/newsletter/api/db/dbusers/' + this.id).then((res) => {
            console.log(res)
            self.cat = res.data
        })
    }
